...
for variable in variables[1:]:  # Skip 'B_BKGCAT' for plotting
    ...
    # Apply the same binning for all histograms
    hist1 = ROOT.TH1F("hist1", f"{variable} for background_1", 100, min_value, max_value)
    hist2 = ROOT.TH1F("hist2", f"{variable} for signal", 100, min_value, max_value)
    hist3 = ROOT.TH1F("hist3", f"{variable} for background_2", 100, min_value, max_value)
    ...
    legend.AddEntry(hist1, "background_1", "l")
    legend.AddEntry(hist2, "signal", "l")
    legend.AddEntry(hist3, "background_2", "l")
    legend.Draw()
